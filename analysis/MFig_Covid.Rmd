...
#pfrya=ff %>% mutate(variant=if_else(str_detect(variant,"Omicron"),"Omicron",variant)) %>% filter(percentage*100 > 3)%>% ggplot(aes(x=file,y=percentage,fill=variant)) + geom_col() + theme_minimal() + labs(x="Samples", y="% variant") + theme( axis.text.x=element_blank())
```
