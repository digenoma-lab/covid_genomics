...
samtools_jfc = read.table('join_flow_cells/multiqc_samtools_stats.txt', header = TRUE)
samtools_pfc = read.table('per_flow_cells/multiqc_samtools_stats.txt', header = TRUE)
 #selecting data
...
reads_mapped_pfc<-reads_mapped_pfc[!grepl("97-97|98-98|99-99|100-100|101-101|102-102|103-103|104-104", reads_mapped_pfc$Sample),]
...
```{r, loadreadcountsdups,echo=FALSE}
#barplot
...
```{r, loadfastqc,echo=FALSE}
qc_sequence_quality_jfc=read.table('join_flow_cells/mqc_fastqc_per_base_sequence_quality_plot_1.txt', h=TRUE,sep="\t")
qc_sequence_quality_jfc.T=setNames(data.frame(t(qc_sequence_quality_jfc[,-1])), qc_sequence_quality_jfc[,1])
##add read2
qc_sequence_quality_pfc=read.table('per_flow_cells/mqc_fastp-seq-quality-plot_Read_1_After_filtering.txt', header = TRUE, sep = '\t')
...
```{r mergeplots, echo=FALSE, fig.width=11,fig.height=10}

mplot=(p1|p2)/(p3|p4) +
  plot_annotation(tag_levels = 'A') &
  theme(plot.tag = element_text(size = 15))

pdf("Fig1_Sup.pdf",12,11)
mplot
dev.off()
mplot
